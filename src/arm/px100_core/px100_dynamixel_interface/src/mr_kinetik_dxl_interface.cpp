#include "px100_dynamixel_interface/mr_kinetik_dxl_interface.h"

/// @brief Constructor for the Px100 arm
/// @param node_handle - ROS NodeHandle
MrKinetikDXLInterface::MrKinetikDXLInterface(ros::NodeHandle *node_handle, bool &success)
    : node(*node_handle)
{
  ...
  {
    success = false;
    ROS_FATAL("[px100_dxl_interface] Failed to find all motors specified in the motor_config file after three attempts. Shutting down...");
    return;
  }
  ...
  {
    success = false;
    ROS_FATAL("[px100_dxl_interface] Failed to write configurations to all motors. Shutting down...");
    return;
  }
  ...
  ROS_INFO("[px100_dxl_interface] px100_dxl_interface node is up!");
}

/// @brief Destructor for the MrKinetikDXLInterface
MrKinetikDXLInterface::~MrKinetikDXLInterface(){}
...
void MrKinetikDXLInterface::robot_set_operating_modes(std::string const& cmd_type, std::string const& name, std::string const& mode, std::string const& profile_type, int32_t profile_velocity, int32_t profile_acceleration)
{
  if (cmd_type == "group" && group_map.count(name) > 0)
  {
    ...
    ROS_INFO("[px100_dxl_interface] The operating mode for the '%s' group was changed to %s.", name.c_str(), mode.c_str());
  }
  else if (cmd_type == "single" && motor_map.count(name) > 0)
  {
    ...
    ROS_INFO("[px100_dxl_interface] The operating mode for the '%s' joint was changed to %s.", name.c_str(), mode.c_str());
  }
  else if (cmd_type == "group" && group_map.count(name) == 0 || cmd_type == "single" && motor_map.count(name) == 0)
    ROS_WARN("[px100_dxl_interface] The '%s' joint/group does not exist. Was it added to the motor config file?", name.c_str());
  else
    ROS_ERROR("[px100_dxl_interface] Invalid command for argument 'cmd_type' while setting operating mode.");
}
...
/// @param profile_velocity - passthrough to the Profile_Velocity register on the motor
/// @param profile_acceleration - passthrough to the Profile_Acceleration register on the motor
void MrKinetikDXLInterface::robot_set_joint_operating_mode(std::string const& name, std::string const& mode, std::string const& profile_type, int32_t profile_velocity, int32_t profile_acceleration)
{
  for (auto const& joint_name:sister_map[name])
  {
    dxl_wb.torque(motor_map[joint_name].motor_id, false);
    ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, torqued off.", motor_map[joint_name].motor_id);
  }
  ...
  for (auto const& motor_name:shadow_map[name])
  {
    ...
    ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, read Drive Mode %d.", motor_map[motor_name].motor_id, drive_mode);

    if (drive_mode <= 1 && profile_type == "time")
    {
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Drive_Mode", drive_mode + 4);
      ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, write Drive Mode %d.", motor_map[motor_name].motor_id, drive_mode + 4);
    }
    ...
    if (mode == "position" || mode == "linear_position")
    {
      ...
      ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, set poscontrolmode, pv=%i, pa=%i.", motor_map[motor_name].motor_id, profile_velocity, profile_acceleration);
    }
    else if (mode == "ext_position")
    {
      ...
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Profile_Velocity", profile_velocity);
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Profile_Acceleration", profile_acceleration);
      ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, set extposcontrolmode, pv=%i, pa=%i.", motor_map[motor_name].motor_id, profile_velocity, profile_acceleration);
    ...
    else if (mode == "velocity")
    {
      ...
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Profile_Acceleration", profile_acceleration);
      ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, set velcontrolmode, pa=%i.", motor_map[motor_name].motor_id, profile_acceleration);
    }
    else if (mode == "pwm")
    {
      dxl_wb.setPWMControlMode(motor_map[motor_name].motor_id);
      ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, set pwmcontrolmode.", motor_map[motor_name].motor_id);
    }

    else if (mode == "current")
    {
      dxl_wb.setCurrentControlMode(motor_map[motor_name].motor_id);
      ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, set currentcontrolmode", motor_map[motor_name].motor_id);
    }
    ...
    {
      ROS_ERROR("[px100_dxl_interface] Invalid command for argument 'mode' while setting the operating mode for the %s motor.", motor_name.c_str());
      continue;
    }
  ...
  for (auto const& joint_name:sister_map[name])
  {
    ...
    ROS_DEBUG("[px100_dxl_interface::robot_set_joint_operating_mode] ID: %d, torqued on.", motor_map[joint_name].motor_id);
  }
...
void MrKinetikDXLInterface::robot_torque_enable(std::string const& cmd_type, std::string const& name, bool const& enable)
{
  if (cmd_type == "group" && group_map.count(name) > 0)
  {
    for (auto const& joint_name:group_map[name].joint_names)
      dxl_wb.torque(motor_map[joint_name].motor_id, enable);
    if (enable) ROS_INFO("[px100_dxl_interface] The '%s' group was torqued on.", name.c_str());
    else ROS_INFO("[px100_dxl_interface] The '%s' group was torqued off.", name.c_str());
  }
  else if (cmd_type == "single" && motor_map.count(name) > 0)
  {
    dxl_wb.torque(motor_map[name].motor_id, enable);
    if (enable) ROS_INFO("[px100_dxl_interface] The '%s' joint was torqued on.", name.c_str());
    else ROS_INFO("[px100_dxl_interface] The '%s' joint was torqued off.", name.c_str());
  }
  else if (cmd_type == "group" && group_map.count(name) == 0 || cmd_type == "single" && motor_map.count(name) == 0)
    ROS_WARN("[px100_dxl_interface] The '%s' joint/group does not exist. Was it added to the motor config file?", name.c_str());
  else
    ROS_ERROR("[px100_dxl_interface] Invalid command for argument 'cmd_type' while torquing joints.");
}
...
void MrKinetikDXLInterface::robot_reboot_motors(std::string const& cmd_type, std::string const& name, bool const& enable, bool const& smart_reboot)
{
  ...
  if (cmd_type == "group" && group_map.count(name) > 0)
  {
    for (auto const& joint_name:group_map[name].joint_names)
    {
      ...
      ROS_INFO("[px100_dxl_interface] The '%s' joint was rebooted.", joint_name.c_str());
      if (enable) joints_to_torque.push_back(joint_name);
    }
    if (!smart_reboot)
      ROS_INFO("[px100_dxl_interface] The '%s' group was rebooted.", name.c_str());
  }
  else if (cmd_type == "single" && motor_map.count(name) > 0)
  {
    dxl_wb.reboot(motor_map[name].motor_id);
    ROS_INFO("[px100_dxl_interface] The '%s' joint was rebooted.", name.c_str());
    if (enable) joints_to_torque.push_back(name);
  }
  else if (cmd_type == "group" && group_map.count(name) == 0 || cmd_type == "single" && motor_map.count(name) == 0)
    ROS_WARN("[px100_dxl_interface] The '%s' joint/group does not exist. Was it added to the motor config file?", name.c_str());
  else
    ROS_ERROR("[px100_dxl_interface] Invalid command for argument 'cmd_type' while rebooting motors.");
...
void MrKinetikDXLInterface::robot_write_commands(std::string const& name, std::vector<float> commands)
{
  if (commands.size() != group_map[name].joint_num)
  {
    ROS_ERROR("[px100_dxl_interface] Number of commands (%ld) does not match the number of joints in group '%s' (%d). Will not execute.", commands.size(), name.c_str(), group_map[name].joint_num);
    return;
  }
  ...
  if (mode == "position" || mode == "ext_position" || mode == "current_based_position" || mode == "linear_position")
  {
    for (size_t i{0}; i < commands.size(); i++)
    {
      ...
      ROS_DEBUG("[px100_dxl_interface::robot_write_commands] ID: %d, writing %s command %d.", group_map[name].joint_ids.at(i), mode.c_str(), dynamixel_commands[i]);
    }
  ...
  else if (mode == "pwm")
  {
    ...
    ROS_ERROR("[px100_dxl_interface] Invalid command for argument 'mode' while commanding joint group.");
}
...
void MrKinetikDXLInterface::robot_write_joint_command(std::string const& name, float command)
{
  ...
  if (mode == "position" || mode == "ext_position" || mode == "current_based_position" || mode == "linear_position")
  {
    ...
    ROS_DEBUG("[px100_dxl_interface::robot_write_joint_command] ID: %d, writing %s command %f.", motor_map[name].motor_id, mode.c_str(), command);
  ...
  else if (mode == "pwm")
  {
    ...
    ROS_ERROR("[px100_dxl_interface] Invalid command for argument 'mode' while commanding joint.");
}
...
void MrKinetikDXLInterface::robot_set_motor_pid_gains(std::string const& cmd_type, std::string const& name, std::vector<int32_t> const& gains)
{
  ...
  for (auto const& name : names)
  {
    ...
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains] ID: %d, writing gains:", motor_map[name].motor_id);
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains]         Pos_P: %i", gains.at(0));
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains]         Pos_I: %i", gains.at(1));
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains]         Pos_D: %i", gains.at(2));
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains]         FF_1: %i", gains.at(3));
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains]         FF_2: %i", gains.at(4));
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains]         Vel_P: %i", gains.at(5));
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_pid_gains]         Vel_I: %i", gains.at(6));
...
void MrKinetikDXLInterface::robot_set_motor_registers(std::string const& cmd_type, std::string const& name, std::string const& reg, int32_t const& value)
{
  std::vector<std::string> names;
  if (cmd_type == "group")
    names = group_map[name].joint_names;
  ...
  for (auto const& name : names)
  {
    ROS_DEBUG("[px100_dxl_interface::robot_set_motor_registers] ID: %d, writing reg: %s, value: %d.", motor_map[name].motor_id, reg.c_str(), value);
...
void MrKinetikDXLInterface::robot_get_motor_registers(std::string const& cmd_type, std::string const& name, std::string const& reg, std::vector<int32_t> &values)
{
  std::vector<std::string> names;
  if (cmd_type == "group")
    names = group_map[name].joint_names;
  ...
  if (goal_reg == NULL)
  {
    ROS_ERROR("[px100_dxl_interface] Could not get '%s' Item Info. Did you spell the register name correctly?", reg.c_str());
    return;
  }
  ...
  for (auto const& name : names)
  {
    ...
    {
      ROS_ERROR("[px100_dxl_interface] %s", log);
      return;
    }
    else
    {
      ROS_DEBUG("[px100_dxl_interface::robot_get_motor_registers] ID: %d, reading reg: %s, value: %d.", motor_map[name].motor_id, reg.c_str(), value);
    }
...
void MrKinetikDXLInterface::robot_get_joint_states(std::string const& name, std::vector<float> *positions, std::vector<float> *velocities, std::vector<float> *effort)
{
  for (auto const& joint_name : group_map[name].joint_names)
  {
    ...
    ROS_DEBUG("[px100_dxl_interface::robot_get_joint_states] ID: %ld, got joint state.", js_index_map[joint_name]);
  }
}
...
void MrKinetikDXLInterface::robot_get_joint_state(std::string const& name, float *position, float *velocity, float *effort)
{
  ...
  ROS_DEBUG("[px100_dxl_interface::robot_get_joint_state] ID: %ld, got joint state.", js_index_map[name]);
}
...
float MrKinetikDXLInterface::robot_convert_linear_position_to_radian(std::string const& name, float const& linear_position)
{
...
float MrKinetikDXLInterface::robot_convert_angular_position_to_linear(std::string const& name, float const& angular_position)
{
  float arm_length = gripper_map[name].arm_length;
  float horn_radius = gripper_map[name].horn_radius;
...
bool MrKinetikDXLInterface::robot_get_motor_configs(void)
{
  ...
  {
    ROS_FATAL("[px100_dxl_interface] Motor Config file was not found or has a bad format. Shutting down...");
    ROS_FATAL("[px100_dxl_interface] YAML Error: '%s'", error.what());
    return false;
  }
  if (motor_configs.IsNull())
  {
    ROS_FATAL("[px100_dxl_interface] Motor Config file was not found. Shutting down...");
    return false;
  }
  ...
  try
  {
    ...
    ROS_INFO("[px100_dxl_interface] Loaded mode configs from '%s'.", mode_configs_file.c_str());
  }
  catch (YAML::BadFile &error)
  {
    ROS_ERROR("[px100_dxl_interface] Motor Config file was not found or has a bad format. Shutting down...");
    ROS_ERROR("[px100_dxl_interface] YAML Error: '%s'", error.what());
    return false;
  }
  if (mode_configs.IsNull())
    ROS_INFO("[px100_dxl_interface] Mode Config file is empty.");
  ...
  ROS_INFO("[px100_dxl_interface] Loaded motor configs from '%s'.", motor_configs_file.c_str());
  return true;
}
...
bool MrKinetikDXLInterface::robot_init_port(void)
{
  if (!dxl_wb.init(port.c_str(), BAUDRATE))
  {
    ROS_FATAL("[px100_dxl_interface] Failed to open port at %s. Shutting down...", port.c_str());
    return false;
  }
  return true;
}
...
bool MrKinetikDXLInterface::robot_ping_motors(void)
{
  ...
  {
    ROS_INFO(
      "[px100_dxl_interface] Pinging all motors specified in the motor_config file. (Attempt %ld/3)",
      cntr_ping_motors+1);
    for (auto const& motor:motor_map)
    {
      ...
      {
        ROS_ERROR(
          "[px100_dxl_interface]\tCan't find DYNAMIXEL ID: %2.d, Joint Name: '%s':\n\t\t  '%s'",
          motor.second.motor_id, motor.first.c_str(), log);
        found_all_motors = false;
      }
      else
        ROS_INFO(
          "[px100_dxl_interface]\tFound DYNAMIXEL ID: %2.d, Model: '%s', Joint Name: '%s'.",
...
bool MrKinetikDXLInterface::robot_load_motor_configs(void)
{
  ros::param::param<bool>("~load_configs", load_configs, true);
  if (load_configs)
  {
    ROS_INFO(
      "[px100_dxl_interface] Writing startup register values to EEPROM. This only needs to be done once on a "
    ...
    for (auto const& motor_info:motor_info_vec)
    {
      ...
      {
        ROS_ERROR(
          "[px100_dxl_interface] Failed to write value[%d] on items[%s] to [ID : %d]",
    ...
    ROS_INFO("[px100_dxl_interface] Skipping writing startup register values to EEPROM.");
  return true;
}
...
void MrKinetikDXLInterface::robot_init_controlItems(void)
{
  ...
  if (!goal_position)
    ROS_ERROR("[px100_dxl_interface] Could not get 'Goal_Position' Item Info");
  ...
  if (!goal_velocity)
    ROS_ERROR("[px100_dxl_interface] Could not get 'Goal_Velocity' or 'Moving_Speed' Item Info");
  ...
  if (!goal_current)
    ROS_INFO(
      "[px100_dxl_interface] Could not get 'Goal_Current' Item Info. This message can be "
  ...
  if (!goal_pwm)
    ROS_ERROR("[px100_dxl_interface] Could not get 'Goal_PWM' Item Info");

  const ControlItem *present_position = dxl_wb.getItemInfo(motor_id, "Present_Position");
  if (!present_position)
    ROS_ERROR("[px100_dxl_interface] Could not get 'Present_Position' Item Info");
  ...
  if (!present_velocity)
    ROS_ERROR("[px100_dxl_interface] Could not get 'Present_Velocity' or 'Present_Speed' Item Info");
  ...
  if (!present_current)
    ROS_ERROR("[px100_dxl_interface] Could not get 'Present_Current' or 'Present_Load' Item Info");
...
void MrKinetikDXLInterface::robot_init_SDK_handlers(void)
{
  if (!dxl_wb.addSyncWriteHandler(control_items["Goal_Position"]->address, control_items["Goal_Position"]->data_length))
    ROS_ERROR("[px100_dxl_interface] Failed to add SyncWriteHandler for Goal_Position.");

  if (!dxl_wb.addSyncWriteHandler(control_items["Goal_Velocity"]->address, control_items["Goal_Velocity"]->data_length))
    ROS_ERROR("[px100_dxl_interface] Failed to add SyncWriteHandler for Goal_Velocity.");
  ...
  if (control_items["Goal_Current"])
  {
    if (!dxl_wb.addSyncWriteHandler(control_items["Goal_Current"]->address, control_items["Goal_Current"]->data_length))
      ROS_ERROR("[px100_dxl_interface] Failed to add SyncWriteHandler for Goal_Current.");
  }
  else
    ROS_INFO("[px100_dxl_interface] SyncWriteHandler for Goal_Current not added as it's not supported.");

  if (!dxl_wb.addSyncWriteHandler(control_items["Goal_PWM"]->address, control_items["Goal_PWM"]->data_length))
    ROS_ERROR("[px100_dxl_interface] Failed to add SyncWriteHandler for Goal_PWM.");
  ...
  if (dxl_wb.getProtocolVersion() == 2.0f)
  {
    ...
    */
    ...
    if (!dxl_wb.addSyncReadHandler(start_address, read_length))
      ROS_ERROR("[px100_dxl_interface] Failed to add SyncReadHandler");
  }
}

/// @brief Loads a 'mode_configs' yaml file containing desired operating modes and sets up the motors accordingly
void MrKinetikDXLInterface::robot_init_operating_modes(void)
{
  YAML::Node all_shadows = motor_configs["shadows"];
  for (YAML::const_iterator master_itr = all_shadows.begin(); master_itr != all_shadows.end(); master_itr++)
  {
...
void MrKinetikDXLInterface::robot_init_publishers(void)
{
  if (pub_states)
...
void MrKinetikDXLInterface::robot_init_subscribers(void)
{
  sub_command_group = node.subscribe("commands/joint_group", 5, &MrKinetikDXLInterface::robot_sub_command_group, this);
  sub_command_single = node.subscribe("commands/joint_single", 5, &MrKinetikDXLInterface::robot_sub_command_single, this);
  sub_command_traj = node.subscribe("commands/joint_trajectory", 5, &MrKinetikDXLInterface::robot_sub_command_traj, this);
}

/// @brief Initialize ROS Services
void MrKinetikDXLInterface::robot_init_services(void)
{
  srv_torque_enable = node.advertiseService("torque_enable", &MrKinetikDXLInterface::robot_srv_torque_enable, this);
  srv_reboot_motors = node.advertiseService("reboot_motors", &MrKinetikDXLInterface::robot_srv_reboot_motors, this);
  srv_get_robot_info = node.advertiseService("get_robot_info", &MrKinetikDXLInterface::robot_srv_get_robot_info, this);
  srv_operating_modes = node.advertiseService("set_operating_modes", &MrKinetikDXLInterface::robot_srv_set_operating_modes, this);
  srv_motor_gains = node.advertiseService("set_motor_pid_gains", &MrKinetikDXLInterface::robot_srv_set_motor_pid_gains, this);
  srv_set_registers = node.advertiseService("set_motor_registers", &MrKinetikDXLInterface::robot_srv_set_motor_registers, this);
  srv_get_registers = node.advertiseService("get_motor_registers", &MrKinetikDXLInterface::robot_srv_get_motor_registers, this);
}

/// @brief Initialize ROS Timers
void MrKinetikDXLInterface::robot_init_timers(void)
{
  execute_joint_traj = false;
  if (timer_hz != 0)
    tmr_joint_states = node.createTimer(ros::Duration(1.0/timer_hz), &MrKinetikDXLInterface::robot_update_joint_states, this);
  tmr_joint_traj = node.createTimer(ros::Duration(0), &MrKinetikDXLInterface::robot_execute_trajectory, this, true, false);
}

/// @brief Waits until first JointState message is received
void MrKinetikDXLInterface::robot_wait_for_joint_states(void)
{
  if (timer_hz == 0) return;
  ros::Rate r(10);
  while (ros::ok() && joint_states.name.size() == 0)
  {
    ROS_DEBUG("[px100_dxl_interface::robot_wait_for_joint_states] Waiting for joint states...");
    ros::spinOnce();
    r.sleep();
  }
  ROS_DEBUG("[px100_dxl_interface::robot_wait_for_joint_states] Joint states found. Continuing.");
}
...
void MrKinetikDXLInterface::robot_sub_command_group(const px100_dynamixel_interface::JointGroupCommand &msg)
{
...
/// @details - refer to the message definition for details
void MrKinetikDXLInterface::robot_sub_command_single(const px100_dynamixel_interface::JointSingleCommand &msg)
{
...
/// @details - refer to the message definition for details
void MrKinetikDXLInterface::robot_sub_command_traj(const px100_dynamixel_interface::JointTrajectoryCommand &msg)
{
  if (execute_joint_traj)
  {
    ROS_WARN("[px100_dxl_interface] Trajectory rejected since joints are still moving.");
    return;
  }
  if (msg.traj.points.size() < 2)
  {
    ROS_WARN("[px100_dxl_interface] Trajectory has fewer than 2 points. Aborting...");
    return;
  }
  ...
  if (timer_hz != 0 && msg.traj.points[0].positions.size() == joint_names.size())
  {
    for (size_t i{0}; i < joint_names.size(); i++)
    {
      ...
      {
        ROS_WARN("[px100_dxl_interface] The %s joint is not at the correct initial state.", joint_names.at(i).c_str());
        ROS_WARN("[px100_dxl_interface] Expected state: %.2f, Actual State: %.2f.", expected_state, actual_state);
      }
    }
  }
  joint_traj_cmd = msg;
...
bool MrKinetikDXLInterface::robot_srv_torque_enable(px100_dynamixel_interface::TorqueEnable::Request &req, px100_dynamixel_interface::TorqueEnable::Response &res)
{
...
/// @details - refer to the service definition for details
bool MrKinetikDXLInterface::robot_srv_reboot_motors(px100_dynamixel_interface::Reboot::Request &req, px100_dynamixel_interface::Reboot::Response &res)
{
  if (!robot_srv_validate(req.cmd_type, req.name))
    return false;
...
/// @details - refer to the service definition for details
bool MrKinetikDXLInterface::robot_srv_get_robot_info(px100_dynamixel_interface::RobotInfo::Request &req, px100_dynamixel_interface::RobotInfo::Response &res)
{
  if (!robot_srv_validate(req.cmd_type, req.name))
    return false;
...
/// @details - refer to the service definition for details
bool MrKinetikDXLInterface::robot_srv_set_operating_modes(px100_dynamixel_interface::OperatingModes::Request &req, px100_dynamixel_interface::OperatingModes::Response &res)
{
  if (!robot_srv_validate(req.cmd_type, req.name))
    return false;
...
bool MrKinetikDXLInterface::robot_srv_set_motor_pid_gains(px100_dynamixel_interface::MotorGains::Request &req, px100_dynamixel_interface::MotorGains::Response &res)
{
  if (!robot_srv_validate(req.cmd_type, req.name))
    return false;
...
/// @details - refer to the service definition for details
bool MrKinetikDXLInterface::robot_srv_set_motor_registers(px100_dynamixel_interface::RegisterValues::Request &req, px100_dynamixel_interface::RegisterValues::Response &res)
{
  if (!robot_srv_validate(req.cmd_type, req.name))
    return false;
...
/// @details - refer to the service definition for details
bool MrKinetikDXLInterface::robot_srv_get_motor_registers(px100_dynamixel_interface::RegisterValues::Request &req, px100_dynamixel_interface::RegisterValues::Response &res)
{
  if (!robot_srv_validate(req.cmd_type, req.name))
    return false;
...
void MrKinetikDXLInterface::robot_execute_trajectory(const ros::TimerEvent &e)
{
...
/// @param e - TimerEvent message [unused]
void MrKinetikDXLInterface::robot_update_joint_states(const ros::TimerEvent &e)
{
  const char* log;
...
bool MrKinetikDXLInterface::robot_srv_validate(const std::string &cmd_type, std::string &name)
{
  if (cmd_type == "group") // if group command...
  {
    ...
    {
      ROS_ERROR("[px100_dxl_interface] Invalid service call. Group '%s' does not exist. Was it added to the motor config file?", name.c_str());
      return false;
    }
  }
  else if (cmd_type == "single") // if single joint command...
  {
    ...
    {
      ROS_ERROR("[px100_dxl_interface] Invalid service call. Joint '%s' does not exist. Was it added to the motor config file?", name.c_str());
      return false;
    }
  }
  else // if command type is invalid, error and return false
  {
    ROS_ERROR("[px100_dxl_interface] Invalid service call. cmd_type '%s'. Choices are 'group' or 'single'.", cmd_type.c_str());
    return false;
  }
}
...
{
    ros::init(argc, argv, "mr_kinetik_dxl_interface");
    ros::NodeHandle n;
    bool success = true;
    MrKinetikDXLInterface bot(&n, success);
    if (success)
        ros::spin();
    else
    {
        ROS_FATAL("[px100_dxl_interface] Error");        
        ros::shutdown();
    }
    return 0;
}
